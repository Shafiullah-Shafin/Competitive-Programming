...
template <typename T> using o_mset = tree<T, null_type, less_equal<T>, rb_tree_tag, tree_order_statistics_node_update>;
...
int main() {
  ...
  mse.insert(4);
  mse.insert(2);
  cout << *mse.find_by_order(0) << endl; ///k th element
  cout << mse.order_of_key(2) << endl; ///number of elements less than k
  return 0;
}
